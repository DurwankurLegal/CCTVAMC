...
async def set_tenant_status(db: AsyncSession, tenant_id: UUID, action: str,
    ...
    before = {
        ...
        "scheduled_hard_delete_at": obj.scheduled_hard_delete_at.isoformat() if obj.scheduled_hard_delete_at else None,
    }
    obj.status = new_status
    obj.is_active = is_active
    
    if action == "cancel":
        obj.scheduled_hard_delete_at = datetime.now(timezone.utc) + timedelta(days=30)
    elif action == "activate":
        obj.scheduled_hard_delete_at = None
